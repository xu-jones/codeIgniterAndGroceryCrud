...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver {
    ...
    protected function get_text_input($field_info, $value) {
        if ($field_info->extras == 'text_editor') {
            ...
            switch ($editor) {
                case'kindeditor':
                    $this->set_css($this->default_texteditor_path . '/kindeditor-4.1.10/themes/default/default.css');
                    $this->set_js_lib($this->default_texteditor_path . '/kindeditor-4.1.10/kindeditor-min.js');
                    $this->set_js_lib($this->default_texteditor_path . '/kindeditor-4.1.10/lang/zh_CN.js');
                    $this->set_js_config($this->default_javascript_path . '/jquery_plugins/config/jquery.kindeditor-4.1.10.config.js');
                    $this->set_js_config($this->default_javascript_path . '/jquery_plugins/config/jquery.kindeditor-4.1.10.config_multiimage.js');
                    break;
            }

            $class_name = $this->config->text_editor_type == 'mini-texteditor' ? 'mini-texteditor' : 'multiimage-texteditor';
        ...
        switch ($this->state_code) {
            ...
            case 11://upload_file
                ...
                if(isset($upload_result[0]->name)){
                    ...
                    $putExtra->Crc32 = 1;
                    list($ret, $err) = Qiniu_PutFile($upToken, $uploadName, $file, $putExtra);
                    if ($err !== null) {
